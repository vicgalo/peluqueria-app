...
export default function ServiciosPage() {
  ...
  const [eActive, setEActive] = useState("30");
  ...
  async function addService() {
    ...
    if (Number.isNaN(total) || total <= 0) return alert("Duración total inválida.");
    if (Number.isNaN(act) || act <= 0) return alert("Duración activa inválida.");
    ...
    <main className="space-y-4">
      ...
      <div className="border rounded-xl bg-white p-4 space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <input
            className="border rounded-md p-2"
            placeholder="Duración activa (min)"
            value={active}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-xl p-4 space-y-3 shadow-2xl ring-1 ring-black/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <input className="border rounded-md p-2" value={eName} onChange={(e) => setEName(e.target.value)} />
              <input className="border rounded-md p-2" value={ePrice} onChange={(e) => setEPrice(e.target.value)} />
              <input className="border rounded-md p-2" value={eDuration} onChange={(e) => setEDuration(e.target.value)} />
              <input className="border rounded-md p-2" value={eActive} onChange={(e) => setEActive(e.target.value)} />
            </div>
